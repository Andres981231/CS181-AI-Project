...
def mainGame(movementInfo):
    ...
    upperPipes = [
        #origin x + 200
        {'x': SCREENWIDTH - (SCREENWIDTH / 2)+ 57, 'y': newPipe1[0]['y']},
        {'x': SCREENWIDTH +57, 'y': newPipe2[0]['y']},
    ]

    # list of lowerpipe
    lowerPipes = [
        {'x': SCREENWIDTH - (SCREENWIDTH / 2)+57, 'y': newPipe1[1]['y']},
        {'x': SCREENWIDTH +57, 'y': newPipe2[1]['y']},
    ]
    ...
    while True:
        ...
        #sample
        result = getNextState(playerx,playery,playerVelY,playerMaxVelY,playerMinVelY,playerAccY,playerFlapped,lowerPipes,upperPipes)
        nextstate, reward = result[0], result[1]
        sample = reward + discount* computeValueFromQvalues(nextstate)
        Qvalues[(state,playerFlapped)] = (1- alpha)*getQvalue(state,playerFlapped) + alpha*sample
...
def computeValueFromQvalues(nextstate):
    ...
    for action in actions:
        if getQvalue(nextstate, action) > maxv:
            maxv = getQvalue(nextstate, action)
    return maxv
...
def computeActionFromQvalues(state):
    ...
    actions = [False,True]
    for action in actions:
        if getQvalue(state, action) > maxv:
            maxv = getQvalue(state, action)
            best_action = action
    return best_action

def getNextState(playerx,playery,playerVelY,playerMaxVelY,playerMinVelY,playerAccY,playerFlapped,lowerPipes,upperPipes):
    if playerVelY < playerMaxVelY and not playerFlapped:
        playerVelY += playerAccY
    ...
    utemp, ltemp = [upperPipes[0].copy(),upperPipes[1].copy()], [lowerPipes[0].copy(),lowerPipes[1].copy()]
    for uPipe, lPipe in zip(utemp,ltemp):
        uPipe['x'] += -4
        lPipe['x'] += -4
    '''
    crashTest = checkCrash({'x': playerx, 'y': playery, 'index': 1},
                               utemp, ltemp) 
